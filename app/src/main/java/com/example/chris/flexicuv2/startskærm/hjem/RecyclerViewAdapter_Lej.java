...
public class RecyclerViewAdapter_Lej extends RecyclerView.Adapter<RecyclerViewAdapter_Lej.ViewHolder>  {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, final int i) {
        ...
        if(index<singleton.getMineLejIndgåedeAftaler().size()) {


            if (i < singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().size()) {
                if (singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).isAftaleIndgået()) {
                    System.out.println("HEEY");
                    int arbejdsdage = udregnArbejdsdage(singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).getLejerStartDato().toString(), singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).getLejerSlutDato().toString());
                    int timeloen = Integer.parseInt(singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).getLejPris());
                    System.out.println(arbejdsdage + ", " + timeloen);
                    double loen = udregnPriser(timeloen, arbejdsdage, 7.4);
                    viewHolder.arbejdeområde.setText(singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).getMedarbejder().getArbejdsomraade());
                    viewHolder.navn.setText(singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).getMedarbejder().getNavn());
                //    viewHolder.virksomhed.setText(singleton.getMineLejIndgåedeAftaler().get(index).getUdlejer().getVirksomhedsnavn());
                    viewHolder.periode.setText(singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).getUdlejerStartDato().replace(" ", "") + " - " + singleton.getMineLejIndgåedeAftaler().get(index).getForhandlinger().get(i).getUdlejerSlutDato().replace(" ", ""));
    ...
    public int getItemCount() {
        return singleton.getMineLejIndgåedeAftaler().size();
    }
